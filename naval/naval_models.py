        ...
        with torch.no_grad():
            ...
            for k, (predi, T) in enumerate(zip(self.pred, self.temporal)):
                if T.type_var>0.5:
                    ...
                    formula_T.append('G')
                formula_time.append([int(self.t1[k]),int(self.t2[k])])
                d = predi.dim
    ...
    def translate_formula(self):
        ...
        with torch.no_grad():
            ...
            for k, (li, T) in enumerate(zip(self.logical1, self.temporal)):
                ...
                formula_i = formula_i + '['+str(int(self.t1[k]))+','+str(int(self.t2[k]))+']'
    ...
    def translate_formula(self):
        ...
        with torch.no_grad():
            ...
            for k, (predi, T) in enumerate(zip(self.pred, self.temporal)):
                formula_i = []
                if T.type_var>0.5:
                    formula_i = 'F'+'['+str(int(self.t1[k]))+','+str(int(self.t2[k]))+']'
                else:
                    formula_i = 'G'+'['+str(int(self.t1[k]))+','+str(int(self.t2[k]))+']'
                d = predi.dim
